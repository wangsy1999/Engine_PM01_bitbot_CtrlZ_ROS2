...
namespace z
{
    ...
    class MotorPDControlWorker2 : public AbstractWorker<SchedulerType>
    {
        /// @brief 电机数据类型
    ...
    public:
         ...
         * @param root_cfg 配置文件
         */
        MotorPDControlWorker2(SchedulerType* scheduler, const nlohmann::json& root_cfg)
        ...
        void TaskRun()
        {
            ...
            this->Scheduler->template GetData<"TargetMotorPosition2">(TargetMotorPos);

            MotorValVec TargetMotorVel;
            this->Scheduler->template GetData<"TargetMotorVelocity2">(TargetMotorVel);

            MotorValVec CurrentMotorPos;
            this->Scheduler->template GetData<"CurrentMotorPosition2">(CurrentMotorPos);

            MotorValVec CurrentMotorVel;
            this->Scheduler->template GetData<"CurrentMotorVelocity2">(CurrentMotorVel);
            ...
            this->Scheduler->template SetData<"TargetMotorTorque2">(Torque);
        }
        void DisablePD() {
            Kp = MotorValVec::zeros();
            Kd = MotorValVec::zeros();
        }
    private:
        /// @brief 位置控制参数
        MotorValVec Kp;
    ...
    class MotorControlWorker2 : public AbstractWorker<SchedulerType>
    {
        /// @brief 电机数据类型
        using MotorValVec = math::Vector<MotorPrecision, JointNumber>;
    public:
         ...
         * @param Joints 电机指针数组
         */
        MotorControlWorker2(SchedulerType* scheduler, const nlohmann::json& root_cfg, const std::array<JointType, JointNumber>& Joints)
            :AbstractWorker<SchedulerType>(scheduler),
            Joints(Joints)
        {
            nlohmann::json cfg = root_cfg;
            this->PrintSplitLine();
            std::cout << "MotorControlWorker2" << std::endl;
        ...
        void TaskCreate()
        {
            ...
            this->Scheduler->template SetData<"CurrentMotorVelocityRaw2">(MotorVel);
            this->Scheduler->template SetData<"CurrentMotorPositionRaw2">(MotorPos);
            this->CurrentMotorVel = (*JointVelFilter)(MotorVel);
            this->CurrentMotorPos = (*JointPosFilter)(MotorPos);

            this->Scheduler->template SetData<"CurrentMotorVelocity2">(CurrentMotorVel);
            this->Scheduler->template SetData<"CurrentMotorPosition2">(CurrentMotorPos);
            this->Scheduler->template SetData<"CurrentMotorTorque2">(MotorTorque);
        ...
        void SetCurrentPositionAsTargetPosition()
        {
            ...
            this->Scheduler->template SetData<"TargetMotorPosition2">(CurrentMotorPos);
            this->Scheduler->template SetData<"TargetMotorVelocity2">(TargetVel);
        }
        ...
        {
            MotorValVec MotorTorque;
            this->Scheduler->template GetData<"TargetMotorTorque2">(MotorTorque);

            MotorValVec MotorPos;
            this->Scheduler->template GetData<"TargetMotorPosition2">(MotorPos);

            MotorValVec MotorVel;
            this->Scheduler->template GetData<"TargetMotorVelocity2">(MotorVel);

            MotorValVec LimitMotorTorque = math::Vector<MotorPrecision, JointNumber>::clamp(MotorTorque, -TorqueLimit, TorqueLimit);
            this->Scheduler->template SetData<"LimitTargetMotorTorque2">(LimitMotorTorque);
