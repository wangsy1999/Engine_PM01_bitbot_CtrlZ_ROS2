...
namespace z
{
    ...
    class MotorResetPositionWorker2 : public AbstractWorker<SchedulerType>
    {
        /// @brief 电机数据类型
    ...
    public:
         ...
         * @param cfg 配置文件
         */
        MotorResetPositionWorker2(SchedulerType* scheduler, const nlohmann::json& Motor_cfg, const nlohmann::json& reset_cfg)
            ...
            enabled(false)
        {
            ...
            this->PrintSplitLine();
            std::cout << "MotorResetPositionWorker2" << std::endl;
        ...
        void StartReset(size_t epoches = 0)
        {
            ...
            this->Scheduler->template GetData<"CurrentMotorPosition2">(this->TargetPosition);
        ...
        void TaskRun() override
        {
            if (this->enabled)
            {
                if (this->Scheduler->getTimeStamp() < this->ResetCnt)
                {
                    ...
                    this->Scheduler->template SetData<"TargetMotorPosition2">(this->TargetPosition);
                }
                else
                {
                    this->Scheduler->template SetData<"TargetMotorPosition2">(this->DefaultPosition);
                    this->enabled = false;
                }
            }
        }
