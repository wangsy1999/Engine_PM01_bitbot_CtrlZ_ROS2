...
namespace z
{
    ...
    public:HumanLabInferenceWorker(SchedulerType* scheduler, const nlohmann::json& Net_cfg, const nlohmann::json& Motor_cfg)
    ...
    {
        // —— 相位/时钟复位 ——
        phase_acc   = static_cast<InferencePrecision>(0);
        phase_active = false;
        phase_start_t = 0;
        clock_sin = static_cast<InferencePrecision>(0);
        clock_cos = static_cast<InferencePrecision>(1);

        // —— 清空 RNN 状态（输入/输出的 hidden & cell） ——


        // —— 清空输入历史 ——
        // HistoryInputBuffer.clear();

        // —— 清空本轮输入张量，避免旧值残留 ——
        // InputTensor.Array().setZero();

        // —— 将上次动作置零并发布（避免策略接管时突变） ——
        MotorValVec zero_action = MotorValVec::zeros();
        this->Scheduler->template SetData<"NetLastAction">(zero_action);

        // —— 发布默认时钟向量（冻结在 0/1） ——
        this->Scheduler->template SetData<"NetClockVector">(
            z::math::Vector<InferencePrecision, 2>{ static_cast<InferencePrecision>(0),
        ...
        void PreProcess() override
        {
            ...
            // // 当满足门限条件时，开始累积 phase
            // if (cmd_norm >= static_cast<InferencePrecision>(0.1)) {
            //     if (!phase_active) {
            //         phase_active = true;
            ...
            //     // 目标值
            //     constexpr InferencePrecision target_sin = 0.0;
            //     constexpr InferencePrecision target_cos = 1.0;

            //     // 平滑系数 (0~1)，越小回到目标越慢
            //     constexpr InferencePrecision alpha = 1;

            //     clock_sin += alpha * (target_sin - clock_sin);
            //     clock_cos += alpha * (target_cos - clock_cos);
            // }
